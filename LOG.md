# LOG.md — System Changelog & Learning Journal

> Append a dated entry every session. Newest at top. Capture **what changed and why**,
> plus any new insight, gap identified, or next action.
...
- Added/changed: Built full Obsidian vault structure (`/raw`, `/wiki`, `/templates`),
  ...
  strategy hubs, ingest + wiki templates, and folder READMEs.
- Learned/insight: Vault is greenfield — zero raw inputs and zero entities so far. The
  highest-leverage first move is to capture John's real objections, scripts, and best
  realtor/builder relationships from his head into raw files.
- Gaps/opportunities: No client data, no role-model frameworks, no content library yet.
  Everything in the strategy hubs is scaffolding/hypothesis until validated by John.
- Next action: Run interview mode to extract (1) top first-time-buyer objections,
  (2) what makes his best realtor partnerships work, (3) his current lead sources.
  Then create the first raw files and promote insights into `/wiki/concepts`.
